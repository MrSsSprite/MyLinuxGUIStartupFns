...
get_battery_charging_status() {
	...
	then
      ...
      if   [ "$battery_percent" -ge 95 ]; then
         ...
         echo -e "\Uf0082";   # battery_90
      elif [ "$battery_percent" -ge 75 ]; then
         echo -e "\Uf0081";   # battery_80
      elif [ "$battery_percent" -ge 65 ]; then
         echo -e "\Uf0080";   # battery_70
      elif [ "$battery_percent" -ge 55 ]; then
         echo -e "\Uf007f";   # battery_60
      elif [ "$battery_percent" -ge 45 ]; then
         echo -e "\Uf007e";   # battery_50
      elif [ "$battery_percent" -ge 35 ]; then
         echo -e "\Uf007d";   # battery_40
      elif [ "$battery_percent" -ge 25 ]; then
         echo -e "\Uf007c";   # battery_30
      elif [ "$battery_percent" -ge 15 ]; then
         echo -e "\Uf007b";   # battery_20
      elif [ "$battery_percent" -ge  5 ]; then
         echo -e "\Uf007a";   # battery_10
      else
         echo -e "\Uf008e";   # battery_outline
      fi
...
xsetroot -name "  $(print_mem)M 󰇚$vel_recv 󰕒$vel_trans $(dwm_alsa) [$(print_bat)]$(show_record) $(print_date) "
